import { useNavigate } from "react-router-dom";
import { useState, useEffect } from 'react';
import htmll from './Variavel';
export default function Habilidades(){
    //let htmll ='data:image/png;base64,';
    const navigate = useNavigate();
    const [cor,setCor] =useState('bodyverde')
    const [faixa,setFaixa] =useState('faixa')
    const [barrah,setbarrah] =useState('barraimg')
    const [inf,setinf] =useState('informacao')
    const [infimg,setinfimg] =useState('.')
    const [fechar,setFechar] =useState('fecharinf2')
    function informe(img){
        setinfimg(img)
        setinf('informacao2')
        setFechar('fecharinf')
        
    }
    function fecharinf(){
        setFechar('fecharinf2')
        setinf('informacao')
    }
    function voltar(){
        navigate("/")
    }
    function mudarCor(){
        if(cor ==='bodyverde'){
            setCor('newbodyverde')
            setFaixa('faixa2')
            setbarrah('barraimg1')
        }else{
            setCor('bodyverde')
            setFaixa('faixa')
            setbarrah('barraimg')
        }
       
    }
    return( <> 
     <div className={cor}>
     <div onClick={fecharinf} className={fechar}></div>
        <div className={inf}>
            <img className='imgf' src={infimg}></img>
            <div className="inftexto"> <p> Colocar texto depois</p> </div>
        </div>
        
        <div className="barraverde">
            <div onClick={voltar} className='homeico1'><ion-icon  size="large" name="home"></ion-icon></div>
            <div onClick={mudarCor} className='homeico2'><ion-icon  className='homeico2' size="large" name="contrast"></ion-icon></div> 
        </div>
        <div className={faixa}>
            <p> Habilidades</p>
        </div>
        
        <div className='barrahab'>
            <div className="barraTitulo"> Front-end</div>
            <div className={barrah}>
                <img className='imgh' onClick={()=> informe(htmll[1])} src={htmll[1]}></img>
                <img className='imgh' onClick={()=> informe(htmll[0])} src={htmll[0]}></img>
                <img className='imgh' onClick={()=> informe(htmll[2])} src={htmll[2]}></img>
                <img className='imgh' onClick={()=> informe(htmll[3])} src={htmll[3]}></img>
                
            </div>
            
        </div>
        <div className='barrahab'>
            <div className="barraTitulo"> Back-end</div>
            <div className={barrah
  
            
            }>
                <img className='imgh'  onClick={()=> informe(htmll[4])} src={htmll[4]}></img>
                <img className='imgh'  onClick={()=> informe(htmll[8])} src={htmll[8]}></img>
                <img className='imgh'  onClick={()=> informe(htmll[5])} src={htmll[5]}></img>
                <img className='imgh'  onClick={()=> informe(htmll[6])} src={htmll[6]}></img>
                <img className='imgh'  onClick={()=> informe(htmll[7])}src={htmll[7]}></img>
                <img className='imgh'  onClick={()=> informe(htmll[9])}src={htmll[9]}></img>
                <img className='imgh'  onClick={()=> informe(htmll[10])}src={htmll[10]}></img>
                <img className='imgh'  onClick={()=> informe(htmll[11])}src={htmll[11]}></img>
                
            </div>
         
        </div>
        
     </div>
        
    </>)
    }